...
def create_network(t_low: int64, t_upper: int64, sx_df: DataFrame, index: int, part1_output_dir: Path, part1_cache_dir: Path):
    logging.debug(
...
def run_part1(run_part1_input: RunPart1Input):
    ...
    for index, t_low in enumerate(time_spans):
        if index < len(time_spans) - 1 and (index < constants.SHOW_N or (index >= mid and index < mid + constants.SHOW_N) or index >= len(time_spans) - 1 - constants.SHOW_N):
